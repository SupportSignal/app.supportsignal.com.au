...
- **Impact**: Company admin from Company A could manage sites in Company B
- **Fix**: Changed all permission checks to `PERMISSIONS.MANAGE_ALL_COMPANIES` (system admin only)
- **Files Updated**: `apps/convex/sites/admin.ts`
- **Functions Fixed**: listSites, createSite, getSiteDetails, updateSite, deleteSite
...
**Lesson Learned**: [UI Component Infrastructure Debugging](../lessons-learned/ui-component-infrastructure-debugging.md)
