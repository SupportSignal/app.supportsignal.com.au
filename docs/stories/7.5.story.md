...
**Root Cause**: Same pattern as Story 7.4 bug. The useEffect populates form data but doesn't clear validation errors, causing the Select component to not recognize the value.

**Fix Applied**: Added `setErrors({})` to useEffect when loading company data (lines 44-54).
...
|-----------|--------|-------|
| SAT-2.1: Company Edit Form Display | ✅ **PASS** | Status field now auto-populated |
